...
class StandaloneMock:
    ...
    def print_rocket_status(self):
        ...
        self._logger.info(
            f"    - Oxidizer Level: {self.sensors['oxidizer_level']:.1f}%"
        )
        self._logger.info(
            f"    - Oxidizer Pressure: {self.sensors['oxidizer_pressure']:.1f} bar"
        )
        ...
        if _frame.device_type == ids.DeviceID.SERVO:
            ...
            if servo_name:
                if _frame.operation == ids.OperationID.SERVO.value.POSITION:
                    ...
                    self._logger.info(
                        f"{servo_name} position set to {new_position} (was {old_val})"
                    )
                    ...
                    self._logger.warning(
                        f"Unknown servo operation {_frame.operation} for {servo_name}"
                    )
            else:
            ...
            if relay_name:
                if _frame.operation == ids.OperationID.RELAY.value.OPEN:
                    ...
                    self._logger.info(
                        f"{relay_name} relay opened (was {old_val}, now 1)"
                    )
                    ...
                    self.relays[relay_name] = 0
                    self._logger.info(
                        f"{relay_name} relay closed (was {old_val}, now 0)"
                    )
                    ...
                    self._logger.warning(
                        f"Unknown relay operation {_frame.operation} for {relay_name}"
                    )
            else:
    ...
    def update_physics(self, dt: float):
        ...
        if self.state == SimulationState.IDLE:
            if self.is_servo_open("fuel_intake"):
                self._logger.warning(
                    "PROPELLANT LOADING VIOLATION: Fuel intake opened before oxidizer is filled!"
                )
                self._logger.warning(
            ...
            if self.is_servo_open("fuel_intake"):
                self._logger.warning(
                    "PROPELLANT LOADING VIOLATION: Fuel intake opened before oxidizer is fully filled!"
                )
                self._logger.warning(
            ...
            if self.is_servo_open("oxidizer_intake"):
                self.sensors["oxidizer_level"] = min(
                    100.0, self.sensors["oxidizer_level"] + dt * 10.0
                )
                self.sensors["oxidizer_pressure"] = min(
                    40.0, self.sensors["oxidizer_pressure"] + dt * 2.0
                )
                ...
                if self.sensors["oxidizer_level"] >= 100.0:
                    ...
                    self.sensors["oxidizer_pressure"] = max(
                        0.0, self.sensors["oxidizer_pressure"] - dt * 1.0
                    )

        elif self.state == SimulationState.OXIDIZER_FILLED:
            if self.relays["oxidizer_heater"] == 1:
                self.sensors["oxidizer_pressure"] = min(
                    90.0, self.sensors["oxidizer_pressure"] + dt * 2.5
                )
                if self.sensors["oxidizer_pressure"] >= 90.0:
                    self.explode(
                        "Oxidizer pressure too high (90 bars) - tank explosion"
                    )
                    return
            else:
                self.sensors["oxidizer_pressure"] = max(
                    30.0, self.sensors["oxidizer_pressure"] - dt * 1.0
                )
            ...
            if self.is_servo_open("fuel_intake"):
                self.sensors["fuel_level"] = min(
                    100.0, self.sensors["fuel_level"] + dt * 10.0
                )
            ...
            if self.fuel_main_open_time and self.oxidizer_main_open_time:
                ...
                if time_diff > 1.0:
                    if self.igniter_start_time:
                        self.explode(
                            "Main valves opened with >1s difference - propellant imbalance explosion"
                        )
                        return

                if self.igniter_start_time:
                    igniter_delay_fuel = abs(
                        self.igniter_start_time - self.fuel_main_open_time
                    )
                    igniter_delay_ox = abs(
                        self.igniter_start_time - self.oxidizer_main_open_time
                    )

                    if igniter_delay_fuel > 1.0 or igniter_delay_ox > 1.0:
                        self.explode(
                            "Igniter started >1s after main valves - engine flooded"
                        )
                        return

                    if self.igniter_start_time < min(
                        self.fuel_main_open_time, self.oxidizer_main_open_time
                    ):
                        self.explode(
                            "Igniter started before main valves - single propellant combustion"
                        )
                        return

                    if self.is_servo_open("fuel_intake") or self.is_servo_open(
                        "oxidizer_intake"
                    ):
                        self.explode(
                            "Intake valves still open during ignition - catastrophic pressure loss"
                        )
                        return
                    ...
                    if pressure < 40.0:
                        self._logger.error(
                            f"Ignition failed: Oxidizer pressure too low ({pressure:.1f} bars) - engine won't ignite"
                        )
                        self.igniter_start_time = None
                        return
                    ...
                    if pressure > 65.0:
                        self.explode(
                            f"Oxidizer pressure too high at ignition ({pressure:.1f} bars) - engine explosion"
                        )
                        return
                    ...
                    if 55.0 <= pressure <= 65.0:
                        self.thrust_multiplier = 1.0
                        self._logger.info(
                            f"Optimal pressure {pressure:.1f} bars - full thrust!"
                        )
                    else:
                        pressure_deviation = min(
                            abs(pressure - 55.0), abs(pressure - 65.0)
                        )
                        self.thrust_multiplier = max(
                            0.5, 1.0 - (pressure_deviation / 15.0) * 0.5
                        )
                        self._logger.warning(
                            f"Suboptimal pressure {pressure:.1f} bars - thrust reduced to {self.thrust_multiplier * 100:.0f}%"
                        )

                    self.state = SimulationState.FLIGHT
                    self._logger.info(
                        f"State: {self.state.value} - Engine ignited successfully!"
                    )
                    self.print_rocket_status()
            ...
            if self.sensors["fuel_level"] > 0:
                if self.relays["parachute"] == 1:
                    self.explode(
                        "Parachute opened while engine is running - structural failure"
                    )
                    return

                burn_rate = dt * 8.0
                self.sensors["fuel_level"] = max(
                    0.0, self.sensors["fuel_level"] - burn_rate
                )
                self.sensors["oxidizer_level"] = max(
                    0.0, self.sensors["oxidizer_level"] - burn_rate
                )
                self.sensors["oxidizer_pressure"] = max(
                    30.0, self.sensors["oxidizer_pressure"] - dt * 3.0
                )
                ...
                if self.relays["parachute"] == 1:
                    if self.velocity > 30.0:
                        self._logger.error(
                            f"Parachute deployed at too high velocity ({self.velocity:.1f} m/s) during ascent - parachute ripped!"
                        )
                        self._logger.error("Continuing ballistic trajectory...")
                    else:
                        self.state = SimulationState.PARACHUTE_DEPLOYED
                        self._logger.info(
                            f"State: {self.state.value} - Early parachute deployment"
                        )
                        self.print_rocket_status()
                        return
                ...
                if self.velocity <= 0 and self.apogee_reached_time is None:
                    ...
                    self._logger.info(
                        f"State: {self.state.value} - Maximum altitude: {self.sensors['altitude']:.2f}m"
                    )
                    self.print_rocket_status()
            ...
            if self.relays["parachute"] == 1:
                ...
                self._logger.info(
                    f"State: {self.state.value} - Parachute not deployed in time!"
                )
                self.print_rocket_status()
            else:
                self.velocity -= 9.81 * dt
            ...
            if self.relays["parachute"] == 1:
                if abs(self.velocity) > 30.0:
                    self._logger.error(
                        f"Parachute deployed at too high velocity ({abs(self.velocity):.1f} m/s) - parachute ripped!"
                    )
                    self._logger.error("Continuing freefall...")
                else:
                    self.state = SimulationState.PARACHUTE_DEPLOYED
                    self._logger.info(
                        f"State: {self.state.value} - Late parachute deployment successful"
                    )
                    self.print_rocket_status()
...
if __name__ == "__main__":
    ...
    standalone_mock = StandaloneMock(
        cl_args.proxy_address,
