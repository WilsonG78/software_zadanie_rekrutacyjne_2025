...
class Frame:
    ...
    payload: Tuple[Number] = field(
        metadata={"bits": 32}, hash=False, compare=False, default_factory=tuple
    )
    ...
    def _ensure_payload_type(self, payload: tuple) -> None:
        ...
        error_msg = (
            f"{self} has wrong payload length (expected {self._valid_payload_len})"
        )
    ...
    def _valid_payload_len(self) -> int:
        return {
            ...
            int(ids.DataTypeID.UINT16INT16): 2,
        }[self.data_type]
    ...
    def _ensure_value_type(self, field_name: str, value: int) -> None:
        try:
            ...
            error_msg = (
                f"{self!r} failed to convert {field_name} of type {type(value)}: "
            )
    ...
    def values_format_str(cls) -> str:
        return "".join(
            "u" + str(f.metadata["bits"]) for f in fields(cls) if f.name != "payload"
        )
    ...
    def payload_format_str(cls, data_type: int) -> str:
        ...
        return {
            int(ids.DataTypeID.NO_DATA): "p32",
            ...
            int(ids.DataTypeID.UINT16INT16): "u16s16",
        }[data_type]
    ...
    def as_mono_str(self) -> str:
        ...
        return " ".join(
            (
                f"{ids.BoardID(self.destination).name:<9}",
    ...
    def __str__(self):
        ...
        return ", ".join(
            (
                f"Frame({ids.BoardID(self.destination).name}",
