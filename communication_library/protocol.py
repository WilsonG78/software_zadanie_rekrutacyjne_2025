...
from communication_library.exceptions import ChecksumMismatchError, ProtocolError
...
class GroundStationProtocol:
    ...
    def _pack(cls, frame: Frame) -> bytes:
        ...
        payload = bitstruct.pack(
            "<" + Frame.payload_format_str(frame.data_type), *frame.payload
        )
    ...
    @classmethod
    def calculate_crc(
        cls, data: bytes, skip_padding: bool = False, return_endianess: str = "little"
    ) -> bytes:
        # padding to a multiple of 4 bytes, because we're using 32bit crc
        if not skip_padding:
            data += (4 - (len(data) % 4)) * b"\x00"
        format_str = int(len(data) / 4) * "I"
        big_endian_data = struct.pack(
            ">" + format_str, *struct.unpack(format_str, data)
        )
        return Crc32Mpeg2.calc(big_endian_data).to_bytes(
            cls.CRC_BYTE_LENGTH, return_endianess
        )
